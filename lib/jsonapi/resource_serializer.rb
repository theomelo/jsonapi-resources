module JSONAPI
  class ResourceSerializer
    ...
    def link_object_to_one(source, relationship, include_linkage)
      ...
      include_meta = relationship.options.fetch(:meta, nil).present?
      link_object_hash = {}
      ...
      link_object_hash[:meta] = relationship_meta(source, relationship) if include_meta
      link_object_hash
    end
    ...
    def relationship_meta(source, relationship)
      meta = relationship.options.fetch(:meta, {})
      meta.is_a?(Proc) ? meta.(source) : meta
    end
